...
function swapPlayers(playerChar) {
    if (playerChar === "X") {
        return "O";
    }
    return "X";
}
function gameButton_event_onclick(event, gameState) {
...
function addButtonsToDOM(buttonsContainer, gameState) {
    for (let i = 0; i < 9; i++) {
        const button = document.createElement("button");
        button.dataset.index = i.toString();
        button.addEventListener("click", (event) => gameButton_event_onclick(event, gameState));
